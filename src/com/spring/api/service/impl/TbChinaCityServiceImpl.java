...
import com.spring.api.dao.TbChinaCityDao;
 ...
 */
 @Service
public class TbChinaCityServiceImpl extends BaseServiceImpl<TbChinaCity, Long> implements TbChinaCityService {
	
	@Resource TbChinaCityDao tbChinaCityDao;

	@Override
	public BaseDao<TbChinaCity, Long> getGenericDao() {
		return tbChinaCityDao;
	}
	...
	public Map<String, Object> findForJson(Map<String, String> params) {
		...
		if (pageSize == 0) {
			List<Map<String,Object>> list = tbChinaCityDao.searchForMap(sql.toString(), values);
			json.put("total", list.size());
			json.put("rows", list);
			return json;
		}else {
			PageBean<Map<String,Object>> pageBean = new PageBean<Map<String,Object>>(page, pageSize);
			pageBean = tbChinaCityDao.searchForMap(sql.toString(), values, pageBean);
	...
	public List<Map<String, Object>> findListById(String pid) {
		...
		sbSql.append("select cid,cname from china_city where pid = "+Integer.valueOf(pid));
		List<Map<String, Object>> list = tbChinaCityDao.searchForMap(sbSql.toString(), null);
		return list;
	}
