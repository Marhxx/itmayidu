...
import com.spring.base.mapper.CamelCaseMapper;
...
public abstract class BaseDaoMysqlImpl<T, ID extends Serializable> extends
	...
	protected int updateById(Map<String, Object> map, boolean... isSelective) {
		if (map == null || map.size() == 0)
			return 0;
		...
		map.remove(this.getPk());
		for (Entry<String, Object> e : map.entrySet()) {
			if(isSelective != null && isSelective.length > 0) {
				if(e.getValue() != null) {
					sb.append(e.getKey());
					sb.append("=?, ");
					values.add(e.getValue());
	...
	public T updateSelective(T t) {
		...
		int count = this.updateById(map,false);
		if (count == 0) {
			throw new DaoException(persistentClass.getName() + "更新失败" + t);
		}
		return t;
...
//		return this.getJdbcTemplate().queryForList(sql, values.toArray());
	...
	@Override
	public List<Map<String, Object>> searchForCamelCaseMap(String sql,
			List<Object> values) {
		if (StringUtils.isEmpty(sql))
			return new ArrayList<Map<String, Object>>();
		if (values == null)
		...
		baseLogger.console("[sql] : " + sql + "\n[values]:" + values);
//		return this.getJdbcTemplate().queryForList(sql, values.toArray());
		return this.getJdbcTemplate().query(sql, values.toArray(), new CamelCaseMapper());
	}
	
	@Override
	public PageBean<Map<String, Object>> searchForCamelCaseMap(String sql,
			List<Object> values, PageBean<Map<String, Object>> pageBean) {
		if (StringUtils.isEmpty(sql)) {
			List<Map<String, Object>> reList = new ArrayList<Map<String, Object>>();
			pageBean.setRowCount(0);
		...
		pageBean.setList(this.searchForCamelCaseMap(sb.toString(), values));
		return pageBean;
	}
