...
public class TbVersionServiceImpl extends BaseServiceImpl<TbVersion, Integer>implements TbVersionService {
	...
	@Override
	public List<Map<String, Object>> findVersionByType(String type) {
		StringBuffer sbSql = new StringBuffer();
		sbSql.append("select v.* ");
		sbSql.append("from tb_version v where v.tv_type = '"+type+"' ");
		sbSql.append("and v.tv_status = 1 ORDER BY v.tv_add_date DESC");
		return tbVersionDao.searchForMap(sbSql.toString(), null);
	}
}
