...
//		log.info("类别:" + json.toString());
	...
	* @throws
	...
	@ResponseBody
	@RequestMapping(value="/list")
	public Map<String, Object> list(String tacCataId,String page,String pageSize){
		Map<String, Object> map = new HashMap<String, Object>();
		List<Map<String, Object>>  list = null;
		
		if(StringUtil.isEmptyNull(tacCataId)){
			map.put("code", 102);
			map.put("message", "参数提交不完整");
			return map;
		}
		if(!StringUtil.isNumber(tacCataId)){
			map.put("code", -1);
		...
		try {
			list = tbAdvertiseContentService.findTbAdvertiseContent(tacCataId,page,pageSize);
			if(list.size()==0){
				map.put("code", "199");
				map.put("message", "暂无数据");
				return map;
			}
			List<Map<String, Object>> listSize = tbAdvertiseContentService.findTbAdvertiseContent(tacCataId,null,null);
			map.put("code", 100);
			...
			logger.error("[TbAdvertiseContentController --> list]:出错,错误原因:" + e.getMessage());
			e.printStackTrace();
			map.put("code", -101);
