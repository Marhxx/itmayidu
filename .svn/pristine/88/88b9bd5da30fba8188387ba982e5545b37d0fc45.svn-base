...
public class TbAdvertiseContentServiceImpl extends BaseServiceImpl<TbAdvertiseContent, Long> implements
	...
	* @see com.spring.backstage.service.TbAdvertiseContentService#findAdvertiseById(java.lang.Long)
	...
	public List<Map<String, Object>> findTbAdvertiseContent(String tacCataId,String page,String pageSize) {
//		return tbAdvertiseContentDao.searchForMap("select a.tac_id as tacId,a.tac_name as tacName,a.tac_phone_image as tacPhoneImage,a.tac_link as tacLink from tb_advertise_content a where a.tac_status=1 and a.tac_cata_id='"+tacCataId+"' ORDER BY a.tac_add_date DESC ", null);
		StringBuffer sbSql = new StringBuffer();
		sbSql.append("SELECT a.* FROM tb_advertise_content a ");
		sbSql.append("WHERE a.tac_status=1 and a.tac_cata_id='"+tacCataId+"' ");
		sbSql.append("ORDER BY a.tac_add_date DESC");
		if(!StringUtils.isBlank(page)){
			sbSql.append(" LIMIT "+(Integer.valueOf(page)-1)*Integer.valueOf(pageSize)+" , "+pageSize);
		}
		return tbAdvertiseContentDao.searchForMap(sbSql.toString(), null);
	}
}
