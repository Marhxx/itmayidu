...
public class TbInformationServiceImpl extends BaseServiceImpl<TbInformation, Long> implements TbInformationService {
	...
	public List<Map<String, Object>> findInfoList(String catalogId, String pageId, String pageCount) {
			...
			if(StringUtil.isEmptyNull(catalogId)){
				sql.append(" SELECT * FROM tb_information WHERE ti_status =1 ");
			}else{
			sql.append(" SELECT * FROM tb_information WHERE ti_status =1 AND ti_cata_id= "+catalogId );
			}
			sql.append(" ORDER BY ti_add_date DESC ");
			logger.debug(sql.toString());
			if(!StringUtil.isEmptyNull(pageId)){
				sql.append( " LIMIT "+(Integer.valueOf(pageId)-1)*Integer.valueOf(pageCount)+" , "+pageCount );
			}
			logger.info(sql.toString());
			return tbInformationDao.searchForMap(sql.toString(), null);
		}

	@Override
	public TbInformation findByCataAll(TbInformation tbInformation) {
		StringBuffer selSql=new StringBuffer();
		selSql.append("SELECT * FROM tb_information WHERE ti_status =1");
